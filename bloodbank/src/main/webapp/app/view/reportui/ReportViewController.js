Ext.define('Bloodbank.view.reportui.ReportViewController', {
	...
	loadDataPoints : function(datapoints, maindatapointpanel)
	{
		...
		/*DatoPoint Panel set random background color*/
		...
		/*Determine whether no.of datapoints are odd or eve so that colspan are set accordingly*/
		if(datapointlist.length%2==1){
			oddFlag=true;
		}
		var totalResultPanel=Object.keys(panelWiseJson).length;
		totalMainPanelWidth=(Math.round(maindatapointpanel.getWidth()/totalResultPanel)-totalResultPanel*2);
		...
		if(maindatapointpanel!=undefined)
		{
			...
			var len=Object.keys(panelWiseJson).length;
			for ( var key in panelWiseJson) 
			{
				len--;
				totalColumns= (panelWiseJson[key].length>7?7:panelWiseJson[key].length);
				var width =  totalMainPanelWidth/ (totalColumns);
				width = width -(totalColumns*5);
				...
				if (panelWiseJson.hasOwnProperty(key)) {
					if(len==0 && oddFlag==true){
						newDataPointPanel = {
								xtype : 'dataPointPanel',
								margin : '0 0 5 0',
								title : (grpflag)?key:undefined,
								dpData : panelWiseJson[key],
								colspan:2,
								width:totalMainPanelWidth
							};
					}else{
						newDataPointPanel = {
								xtype : 'dataPointPanel',
