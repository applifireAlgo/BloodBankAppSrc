Ext.define('Bloodbank.bloodbank.web.com.controller.bloodbank.RegisterDonorToCampMainController', {
     extend: 'Bloodbank.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.RegisterDonorToCampMainController',
     ...
     campLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    ...
                    scope.sender.controller.assignAllComboData(scope.sender, 'camp', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#camp');
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#RegisterDonorToCampTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#RegisterDonorToCampGrid');
               grid.store.add(data);
               var tree = this.view.down('#RegisterDonorToCampTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#RegisterDonorToCamp');
          var grid = this.view.down('#RegisterDonorToCampGrid');
          var tree = this.view.down('#RegisterDonorToCampTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#RegisterDonorToCampGrid');
               ...
               var formPanel = this.getView().up().down("#RegisterDonorToCamp");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#RegisterDonorToCampTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#RegisterDonorToCamp').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#RegisterDonorToCampGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.registerDonorToCampId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#RegisterDonorToCamp').getViewModel().setData(responseData.response.data);
     },
     ...
     saveForm: function(but, evt) {
          ...
          var method;
          if (jsonData.registerDonorToCampId == null) {
               method = 'POST'
          } else {
               method = 'PUT';
               jsonData.systemInfo = {
