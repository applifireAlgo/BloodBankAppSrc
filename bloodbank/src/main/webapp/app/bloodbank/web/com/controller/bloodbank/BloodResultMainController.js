Ext.define('Bloodbank.bloodbank.web.com.controller.bloodbank.BloodResultMainController', {
     extend: 'Bloodbank.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.BloodResultMainController',
     init: function() {
     ...
     donorVisitLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#donorVisit');
                    scope.sender.controller.assignAllComboData(scope.sender, 'donorVisit', response.responseText);
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#BloodResultTree');
               ...
               var compRef = this.view.down('#BloodResultTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.bloodResultId;
          me = this;
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#BloodResult');
          var grid = this.view.down('#BloodResultGrid');
          var tree = this.view.down('#BloodResultTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#BloodResultTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var BloodResult = this.view.down('#BloodResultForm');
               BloodResult.reset();
               var gridPanel = this.view.down('#BloodResultGrid');
          ...
          var formPanel = this.getView().up().down('#BloodResultForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#BloodResultTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#BloodResult').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#BloodResultGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#BloodResultForm').getViewModel().setData(responseData.response.data);
     },
     ...
     saveForm: function(but, evt) {
          ...
          var method;
          if (jsonData.bloodResultId == null) {
               method = 'POST'
          } else {
               method = 'PUT';
               jsonData.systemInfo = {
