Ext.define('Bloodbank.bloodbank.web.com.controller.bloodbank.BloodGroupMainController', {
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#BloodGroupTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#BloodGroupGrid');
               grid.store.add(data);
               var tree = this.view.down('#BloodGroupTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#BloodGroup');
          var grid = this.view.down('#BloodGroupGrid');
          var tree = this.view.down('#BloodGroupTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#BloodGroupGrid');
               ...
               var formPanel = this.getView().up().down("#BloodGroup");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#BloodGroupTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.bloodGroupId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#BloodGroup').getViewModel().setData(responseData.response.data);
     },
