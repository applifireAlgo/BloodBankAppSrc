Ext.define('Bloodbank.bloodbank.web.com.controller.bloodbank.DonationTypeMainController', {
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#DonationTypeTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#DonationTypeGrid');
               grid.store.add(data);
               var tree = this.view.down('#DonationTypeTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#DonationType');
          var grid = this.view.down('#DonationTypeGrid');
          var tree = this.view.down('#DonationTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#DonationTypeGrid');
               ...
               var formPanel = this.getView().up().down("#DonationType");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#DonationTypeTree');
     ...
     loadData: function(responseData) {
          this.view.down('#DonationType').getViewModel().setData(responseData.response.data);
     },
     ...
     saveForm: function(but, evt) {
          ...
          var method;
          if (jsonData.donationTypeId == null) {
               method = 'POST'
          } else {
               method = 'PUT';
               jsonData.systemInfo = {
